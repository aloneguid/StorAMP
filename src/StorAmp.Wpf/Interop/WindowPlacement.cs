...
namespace CloudExplorer.Wpf.Interop
{
   ...
   public static class WindowPlacement
   {
      ...
      private static readonly XmlSerializer _serializer = new XmlSerializer(typeof(WINDOWPLACEMENT));
      ...
      public static void SetPlacement(IntPtr windowHandle, string placementXml)
      {
         ...
         byte[] xmlBytes = _encoding.GetBytes(placementXml);

         try
         {
            using(var memoryStream = new MemoryStream(xmlBytes))
            {
               placement = (WINDOWPLACEMENT)_serializer.Deserialize(memoryStream);
            }
